function lastSurvivor(letters, coords) {
    ...
    // for (let num = 1; num < coords.length; num++) {
  for (num of coords){
    // letters.charAt(num) = ""
    // letters.charAt(letters[num]) = ""
    // var fixedLetters = letters.replace(letters[num],"")
    // var letters = letters.replace(letters[num],"").trim()
...
lastSurvivor(str1,arr1)

for (let i = 0; i < 5; i++){
    console.log("loopying " + (i+1));
}

const names  = ["joe", "diane", "raheem", "lester"]
for (name of names) console.log(name);

const user = {'firstName': 'John', 'lastName': 'Doe'}
myVar = "John John The Don"

console.log(`${myVar}`)
console.log(`${user["firstName"]} ${user["firstName"]} The Don"`)